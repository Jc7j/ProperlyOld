...
export default function MonthlyOverviewDialog() {
  const [open, setOpen] = useState(false)
  const [date, setDate] = useState<Date | undefined>(new Date())
  const [isLoading, setIsLoading] = useState(false)
  const [summarizedData, setSummarizedData] = useState<PropertySummary[]>([])
  const [step, setStep] = useState<'select-month' | 'view-results'>(
    'select-month'
  )
  ...
  const properties = api.property.getMany.useQuery(undefined, {
    enabled: open && !!monthQuery && step === 'view-results',
  })
  ...
  useEffect(() => {
    ...
    const summaries: PropertySummary[] = properties.data.map((property) => {
      ...
      const ownerStatement = ownerStatements.data?.find(
  ...
  const renderDialogContent = () => {
    ...
    return (
      <>
        <DialogTitle>Monthly Property Statement</DialogTitle>
        <DialogBody>
          <div className="flex justify-between items-center mb-4">
            <p className="text-sm text-zinc-500">
              {date ? dayjs(date).format('MMMM YYYY') : 'All Properties'}
            </p>
            <Button
              variant="outline"
